...
var chart = function() {
	var flowDom = $('code.flow');
	flowDom.each(function(i,ele){
		var code = $(this).text();
		
		var flow = flowchart.parse(code);
		var id = 'flow' + i ;
		$(this).parent().before('<div id="' + id + '"></div>');
		$(this).parent().remove();
		flow.drawSVG(id,{
                      // 'x': 30,
                      // 'y': 50,
	...
	// var code = flowDom.text()
	// flowDom.html('');
	// charta = flowchart.parse(code);
	// var a = flowDom.parent().before('<div id="aaa"></div>');
	// console.log(a)
 //          charta.drawSVG(a[0]);
}
//
